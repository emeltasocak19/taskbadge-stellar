...
export type DataKey = {tag: "Task", values: readonly [string, string]} | {tag: "TaskCount", values: readonly [string]} | {tag: "TotalTasks", values: void};
...
export interface Client {
   ...
   */
  has_task: ({user, task}: {user: string, task: string}, options?: MethodOptions) => Promise<AssembledTransaction<boolean>>
   ...
   */
  complete_task: ({user, task}: {user: string, task: string}, options?: MethodOptions) => Promise<AssembledTransaction<u32>>
   ...
   */
  get_task_count: ({user}: {user: string}, options?: MethodOptions) => Promise<AssembledTransaction<u32>>
...
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([ "AAAAAgAAAAAAAAAAAAAAB0RhdGFLZXkAAAAAAwAAAAEAAAAAAAAABFRhc2sAAAACAAAAEwAAABAAAAABAAAAAAAAAAlUYXNrQ291bnQAAAAAAAABAAAAEwAAAAAAAAAAAAAAClRvdGFsVGFza3MAAA==",
        ...
        "AAAAAAAAAAAAAAAPZ2V0X3RvdGFsX3Rhc2tzAAAAAAAAAAABAAAABA==" ]),
      options
    )
  }
  public readonly fromJSON = {
    has_task: this.txFromJSON<boolean>,
        complete_task: this.txFromJSON<u32>,
        get_task_count: this.txFromJSON<u32>,
        get_total_tasks: this.txFromJSON<u32>
  }
}